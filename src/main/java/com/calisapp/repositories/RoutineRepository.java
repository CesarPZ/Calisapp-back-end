...
public interface RoutineRepository extends CrudRepository<Routine, Integer> {
	...
	@Query(value = "SELECT * FROM routines r WHERE r.id in (SELECT routines_id FROM users_routines u WHERE u.user_id = :idUser)", nativeQuery = true)
	List<Routine> findWithUserId(@Param("idUser") Long idUser);
}
